...
export function OrderActions({
  ...
  const [activeAction, setActiveAction] = useState<"fulfill" | "cancel" | null>(
    null,
  );
  const [error, setError] = useState<string | null>(null);

  function call(action: "fulfill" | "cancel") {
    const verb = action === "fulfill" ? "Fulfill" : "Cancel";
    const trailing =
      action === "fulfill"
        ? " The notes you wrote will be saved alongside the order."
        : " Points will be refunded to the ambassador if this was a points order.";
    if (!window.confirm(`${verb} this order?${trailing}`)) return;
    ...
    <div className="space-y-4">
      <label className="block">
        ...
        <span className="text-[12px] text-mute mt-1.5 block">
          Saved when you fulfill or cancel.
        </span>
      </label>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          <Check size={15} />
          {pending && activeAction === "fulfill" ? "Fulfilling…" : "Mark fulfilled"}
        </button>
        <button
          type="button"
          onClick={() => call("cancel")}
